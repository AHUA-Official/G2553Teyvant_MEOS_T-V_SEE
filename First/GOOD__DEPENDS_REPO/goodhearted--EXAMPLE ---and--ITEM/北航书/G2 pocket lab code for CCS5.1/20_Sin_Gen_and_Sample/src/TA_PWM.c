...
* 范       例 : TA0_PWM_Init('A',1,'P','P')TA时钟设为ACLK,通道1和通道2均为超前PWM输出
                   TA0_PWM_Init('S',4,'D','D')TA时钟设为SMCLK/4, 通道1为死区增PWM、通道2为死区减PWM
                   TA0_PWM_Init('A',1,'P',0)TA时钟设为ACLK,通道1超前PWM输出，通道2不作TA用。
****************************************************************************/
...
****************************************************************************/
char TA0_PWM_SetPermill(char Channel,unsigned long int Percent)
{
	unsigned char Mod = 0;
...
* 出口参数：1表示设置成功，0表示参数错误，设置失败。
* 说       明 : 在调用PWM相关函数之前，需要调用该函数设置TA的模式和时钟源。
* 范       例 : TA1_PWM_Init('A',1,'F','F')TA时钟设为ACLK,通道1和通道2均为超前PWM输出
...
****************************************************************************/
char TA1_PWM_SetPermill(char Channel,unsigned long int Percent)
{
	unsigned char Mod;
	switch (Channel)											///先判断出通道的工作模式
		{
	...
	case 2: case 6:			/**死区模式2,6时，需要判断修正死区时间，且同时设定TA1CCR1/2 的值*/
		{
			if((100-2*Percent)<=DEADTIME)	//预留死区时间
				Percent=(100-DEADTIME)/2;
			TA1CCR1=Percent* TA1CCR0/100;
		...
		case 7:
		{
			...
			if(Channel==1) TA1CCR1=Percent* TA1CCR0/100;
			if(Channel==2) TA1CCR2=Percent* TA1CCR0/100;
			break;
		}
		case 3:		//占空比一律为正脉宽，所以需要 TA1CCR0减去占空比
		{
			if(Percent>100)	Percent=100;
			if(Channel==1) TA1CCR1= TA1CCR0-Percent* TA1CCR0/100;
			if(Channel==2) TA1CCR2= TA1CCR0-Percent* TA1CCR0/100;
			break;
		}
		default: return(0);
		}
