/**************************************************
      ...
      来显示当前的温度（精确到0.6C）。利用三极管来直接驱动DS18B20，
      即MSP430进入睡眠时，可以关闭DS18B20，达到省电的目的。

注意：
     1.P1.0，P1.4最好先别急着用,可以利用P1.0（ACLK）外部手表时钟检测是否进入LPM4 
       P1.4 (SMCLK)内部DCO,为了测试是否进入低功耗模式LPM3,要是没有输出方波
       说明已经进入LPM3。
     2.最后进入低功耗的时候，一定要配置好IO口，除键盘,关断三极管P1.0所用的IO口设置输入外，
       其他一定要设置输出，P1.0一定置低，关断三极管9013，
       即关闭DS18B20.最好其他的最好置高电平。
...
///////////////////////硬件连接/////////////////////
      ...
      温度传感器DS18B20的1引脚 即：DS18B20_GND-------三极管9013的3脚（C）
      睡眠按键------------------------------------------------- P2.0 
      唤醒温度按键--------------------------------------------- P2.1
      唤醒分秒按键--------------------------------------------- P2.2
      时间分的调节按键----------------------------------------- P2.3
...
void port_init(void)            //端口初始化
{
  ...
  P1OUT |= BIT0;                     //P1.0用于控制三极管的关断
                                     //高电平时，二极管导通，即DS18B20工作
                                     //低电平时，
...
void stop_lcd(void)      //关闭LCD子程序
{
    ...
    P1DIR=0XFF;        //设置IO
    P1OUT =0XFE;       //P1.0一定要置低，关断三极管，即关闭DS18B20
    P2DIR=0xC0;        //
    P2OUT |=0xC0;      //
}
//timeA定时中断服务程序
