...
{
	 _DINT();
	  P3SEL |= 0x0a;    // GPIO 配置为USCI_B0功能P3.1-SDA  P3.3-SCL
          ...
          U0CTL |= I2CEN;                           // Enable I2C, 7 bit addr,               // 开启I2C
	  _EINT();
}
/******************************************************************************************************
 * 名       称：I2C_Tx_Init()
...
{
	  _DINT();
	  while ((I2CBUSY & I2CDCTL)||(I2CTCTL & I2CSTP)); // 确保总线空闲
	  IE2 &= ~URXIE1;	//关闭Rx中断
	  I2C_State=TX_STATE;
	  IE2 |= UTXIE1;     //允许Tx中断
	  _EINT();	// 开总中断
...
{
	if ((I2CBUSY & I2CDCTL)||(I2CTCTL & I2CSTP))		
          return(0);
 	pTxData = (unsigned char *)p_Tx;   // 更新数据指针
        TxByteCnt = num;                  // 更新剩余发送数据个数
        U0CTL |= MST;       // I2C Tx位, 软件start condition
        I2CTCTL |= I2CSTT;
        _BIS_SR(CPUOFF+GIE);       	  // 进LPM0模式，开总中断
        return(1);
}
/******************************************************************************************************
 * 名       称：I2C_RxFrame()
...
unsigned char I2C_RxFrame(unsigned char *p_Rx,unsigned char num)
{
	...
	pRxData = (unsigned char *)p_Rx;  // 更新数据指针
	 ...
	 U0CTL &= ~MST;
     I2CTCTL |= I2CSTT;          // I2C Rx位, 软件start condition
    _BIS_SR(CPUOFF+GIE);       	    //  进LPM0模式，开总中断
    return(1);
 }
...
void I2C_TxFrame_ISR(void)
{
  ...
  {
    I2CTCTL |= I2CSTP;                   	// 置停止位
    IFG2 &= ~UTXIFG1;                     	// 人工清除标志位（由于没有写Buffer，MCU不会自动清除）
  }
}
/******************************************************************************************************
 * 名       称：I2C_RxFrame_ISR()
...
void I2C_RxFrame_ISR(void)
{
	  if (RxByteCnt == 1)                       			// 只剩1字节没接收时（实际已经在RxBuff里了）
	      I2CTCTL |= I2CSTP;                  		// 软件产生停止位
	  RxByteCnt--;                              				// 待接收字节数递减
	  *pRxData = U0RXBUF;                  		// 存储已接收的数据
	  *pRxData++;												//数据指针移位
}
